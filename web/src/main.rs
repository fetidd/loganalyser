...
impl AppState {
    async fn new() -> anyhow::Result<Self> {
        ...
        env.add_template(
            "spans_waterfall.html",
            include_str!("../templates/spans_waterfall.html"),
        )?;
...
async fn seed_store(store: &MemoryEventStore) -> anyhow::Result<()> {
    ...
    traced.push(
        Event::new_single(
            "auth_check",
            now - Duration::minutes(2) + Duration::milliseconds(5),
            HashMap::from([
                ("user_id".into(), "1007".into()),
                ("result".into(), "ok".into()),
            ]),
            None,
        )
    ...
    traced.push(
        Event::new_single(
            "cache_lookup",
            now - Duration::minutes(2) + Duration::milliseconds(12),
            HashMap::from([
                ("key".into(), "order:42".into()),
                ("hit".into(), "false".into()),
            ]),
            None,
        )
    ...
    traced.push(
        Event::new_single(
            "cache_lookup",
            now - Duration::minutes(2) + Duration::milliseconds(140),
            HashMap::from([
                ("key".into(), "product:prices".into()),
                ("hit".into(), "true".into()),
            ]),
            None,
        )
    ...
    let req2 = Event::new_span(
        ...
        HashMap::from([
            ...
            ("path".into(), "/api/auth/login".into()),
            ("status".into(), "401".into()),
        ]),
    ...
    traced.push(
        Event::new_single(
            ...
            HashMap::from([
                ("user_id".into(), "2031".into()),
                ("result".into(), "denied".into()),
                ("reason".into(), "bad_password".into()),
            ]),
            None,
        )
        .with_parent(req2_id),
    ...
    let job = Event::new_span(
        "background_job",
        now - Duration::hours(3),
        HashMap::from([
            ("job".into(), "report_gen".into()),
            ("status".into(), "ok".into()),
        ]),
