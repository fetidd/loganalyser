...
async fn seed_store(store: &EventStorage) -> anyhow::Result<()> {
    ...
    for i in 0i64..10 {
        events.push(Event::new_single(
            ...
            String::from("2026-08-12 12:32:08 cache_lookup data=123 error=0 miss=false"),
        ));
    }
    ...
    for i in 0i64..8 {
        events.push(Event::new_single(
            ...
            HashMap::from([("user_id".into(), format!("{}", 1000 + i * 7)), ("result".into(), if i % 4 == 0 { "denied" } else { "ok" }.into())]), String::new()));
    }
    ...
    traced.push(Event::new_single("auth_check", now - Duration::minutes(2) + Duration::milliseconds(5), HashMap::from([("user_id".into(), "1007".into()), ("result".into(), "ok".into())]), String::new()).with_parent(req1_id));

    traced.push(Event::new_single("cache_lookup", now - Duration::minutes(2) + Duration::milliseconds(12), HashMap::from([("key".into(), "order:42".into()), ("hit".into(), "false".into())]), String::new()).with_parent(req1_id));
    ...
    traced.push(Event::new_single("cache_lookup", now - Duration::minutes(2) + Duration::milliseconds(140), HashMap::from([("key".into(), "product:prices".into()), ("hit".into(), "true".into())]), String::new()).with_parent(req1_id));
    ...
    traced.push(
        Event::new_single(
            ...
            HashMap::from([("user_id".into(), "2031".into()), ("result".into(), "denied".into()), ("reason".into(), "bad_password".into())]), String::new())
        .with_parent(req2_id),
    );

    // Trace 3: background job with sub-queries
    let job = Event::new_span("background_job", now - Duration::hours(3), HashMap::from([("job".into(), "report_gen".into()), ("status".into(), "ok".into())]), Duration::milliseconds(4200), (String::new(), String::new()));
    let job_id = job.id();
    traced.push(job);
